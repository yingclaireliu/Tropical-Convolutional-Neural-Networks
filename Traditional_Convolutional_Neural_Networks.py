...
# minplus卷积操作
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0):
        ...
        # 卷积核的形状是（卷积核个数（也是输出通道数），输入图片通道数，卷积核长，卷积核宽）
    ...
    def forward(self, x):
        ...
        # 确定输出的形状并用0初始化，输出的形状是（图片数量，输出通道数，长，宽）
...
# MinP_S + MaxP_S
def net1(opt):
    # 需要的输入数据的大小格式(n, c, h, w)，也就是数量，通道，高，宽
    # 本例为torch.Size([batchsize, 1, 28, 28])
    model = nn.Sequential(
...
'''
以下为加载数据函数、训练函数、保存结果函数
'''
def load_data():
    if opt.dataset == 'mnist':
        # 使用torch自带数据集
        transform = transforms.Compose([transforms.ToTensor()])
        #读取数据集，读取的格式是TensorDataset
...
def train_batch(model, loss_func, xb, yb, opt, optimizer=None):
    '''
    在训练阶段要反向传播、优化器step，在测试阶段不启用优化器，只是得到loss
    '''
    # xb = xb.cuda()
    # yb = yb.cuda()
    pred = model(xb.float())
...
def fit(opt, model, loss_func, optimizer, train_dl, test_dl):
    ...
    for epoch in range(opt.epoch):
        ...
        # model.train()\model.eval()作用为：启用\不启用 BatchNormalization 和 Dropout，将BatchNormalization和Dropout置为True\False
        model.train()
        ...
        if test_acc <= maxtest_acc:
            trigger_times += 1
            if trigger_times >= patience and epoch > opt.fitepoch + 15:
...
def options_func():
    ...
    optParser.add_option("--fitepoch", action = "store", type = "int", dest = 'fitepoch', default = 5, help = "the min epoch after using early stopping")
